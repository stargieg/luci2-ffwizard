...
zonedata() {
  ...
  local origin="/tmp/hosts/babelnode"
  UB_CONF=$UB_VARDIR/babelnode.conf

  if [ -f "$UB_TOTAL_CONF" ] && [ -f "$origin" ] ; then
    local longconf dateconf dateoldf
    local dns_add=$UB_VARDIR/babelnode_dns.add
    local dns_del=$UB_VARDIR/babelnode_dns.del
    local dns_new=$UB_VARDIR/babelnode_dns.new
    local dns_old=$UB_VARDIR/babelnode_dns.old
    local origin_new=$UB_VARDIR/babelnode.new
...
UB_LOCK=/tmp/unbound_babelnode.lock
