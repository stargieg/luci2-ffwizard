...
zonedata() {
  ...
  local dhcp_domain="olsr"
  #local origin=$( uci_get dhcp.@dnsmasq[0].leasefile )
  local origin="/tmp/hosts/olsr"
  UB_CONF=$UB_VARDIR/olsr.conf

  if [ -f "$UB_TOTAL_CONF" ] && [ -f "$origin" ] \
  && [ -n "$dhcp_domain" ] ; then
    local longconf dateconf dateoldf
    local dns_add=$UB_VARDIR/olsr_dns.add
    local dns_del=$UB_VARDIR/olsr_dns.del
    local dns_new=$UB_VARDIR/olsr_dns.new
    local dns_old=$UB_VARDIR/olsr_dns.old
    local origin_new=$UB_VARDIR/olsr.new
    ...
    freshstart)
      awk -v conffile=$UB_CONF -v pipefile=$dns_new \
          ...
          -f /usr/lib/unbound/olsr.awk $origin_new
    ...
    increment)
      ...
      awk -v conffile=$UB_CONF -v pipefile=$dns_new \
          -v domain=$dhcp_domain -v bconf=0 \
          -f /usr/lib/unbound/olsr.awk $origin_new
