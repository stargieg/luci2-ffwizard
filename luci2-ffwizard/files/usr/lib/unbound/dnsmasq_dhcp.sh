...
zonedata() {
  ...
  UB_CONF=$UB_VARDIR/dnsmasq_dhcp.conf

  if [ -f "$UB_TOTAL_CONF" ] && [ -f "$origin" ] \
  && [ "$dhcp_link" = "odhcpd" ] && [ -n "$dhcp_domain" ] ; then
    local longconf dateconf dateoldf
    local dns_add=$UB_VARDIR/dnsmasq_dns.add
    local dns_del=$UB_VARDIR/dnsmasq_dns.del
    local dns_new=$UB_VARDIR/dnsmasq_dns.new
    local dns_old=$UB_VARDIR/dnsmasq_dns.old
    local origin_new=$UB_VARDIR/dnsmasq_lease.new
...
UB_LOCK=/tmp/unbound_dnsmasq_dhcp.lock
