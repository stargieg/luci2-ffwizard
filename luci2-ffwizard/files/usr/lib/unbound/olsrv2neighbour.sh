...
zonedata() {
  ...
  local origin="/tmp/hosts/olsrneighbor"
  UB_CONF=$UB_VARDIR/olsrv2neighbor.conf

  if [ -f "$UB_TOTAL_CONF" ] && [ -f "$origin" ] ; then
    local longconf dateconf dateoldf
    local dns_add=$UB_VARDIR/olsrv2neighbor_dns.add
    local dns_del=$UB_VARDIR/olsrv2neighbor_dns.del
    local dns_new=$UB_VARDIR/olsrv2neighbor_dns.new
    local dns_old=$UB_VARDIR/olsrv2neighbor_dns.old
    local origin_new=$UB_VARDIR/olsrv2neighbor.new
...
UB_LOCK=/tmp/unbound_olsrv2neighbor.lock
