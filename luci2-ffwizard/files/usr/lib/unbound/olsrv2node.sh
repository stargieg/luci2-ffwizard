...
zonedata() {
  ...
  local origin="/tmp/hosts/olsrnode"
  UB_CONF=$UB_VARDIR/olsrv2node.conf

  if [ -f "$UB_TOTAL_CONF" ] && [ -f "$origin" ] ; then
    local longconf dateconf dateoldf
    local dns_add=$UB_VARDIR/olsrv2node_dns.add
    local dns_del=$UB_VARDIR/olsrv2node_dns.del
    local dns_new=$UB_VARDIR/olsrv2node_dns.new
    local dns_old=$UB_VARDIR/olsrv2node_dns.old
    local origin_new=$UB_VARDIR/olsrv2node.new
...
UB_LOCK=/tmp/unbound_olsrv2node.lock
